...
<table cellspasing="0" border="0" cellpadding="0" style="margin: 0pt; padding: 0pt; background-color: rgb(255, 255, 255); border-collapse: collapse;">
	<!-- QR-code -->
    <tr>
    	<td><a href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/ticket/view/<?php echo $model->uniq ?>" target="_blank" title=""><img src="http://<?php echo $_SERVER['HTTP_HOST'].$model->qr ?>" alt="<?php echo Events::getEventTitle($model->event_id) ?>" style="margin: 0pt; padding: 0pt; border: 0pt none; display: block; border:1px #cccccc solid;"></a></td>
    </tr>
    <!-- hello, usename -->
    <tr>
    ...
    <tr>
        <?php if(!$eventUniq): ?>
            <td style="background-color:#e5e5e5;"><p style="padding:5px 10px; font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:normal; font-style:normal; color:#333;">
                Вы купили билет на мероприятие под названием «<a target="_blank" title="<?php echo Events::getEventTitle($model->event_id) ?>" href="http://<?php echo $_SERVER['HTTP_HOST'] ?>/events/view/<?php echo $model->event_id ?>"><?php echo Events::getEventTitle($model->event_id) ?></a>», которое состоится <b>
    ...
    <tr>
    	<td style="font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:normal; font-style:normal; color:#000;"><?php echo CHtml::link("Ссылка на билет", "http://".$_SERVER['HTTP_HOST']."/ticket/view/".$model->uniq) ?></td>
    </tr>
    <tr>
