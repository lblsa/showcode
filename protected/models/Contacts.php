...
class Contacts extends CActiveRecord
{
	...
	*/
	...
	public function getTextEmailOfFeedback(){
            ...
            $text .= '<tr>';
            $text .= '<td style="background-image:url(https://' .$_SERVER['HTTP_HOST']. '/images/email/content_bg.png); background-repeat:no-repeat; background-position:left top; padding-top: 0px; padding-right:0; padding-bottom:0; padding-left: 21px;">';
            ...
            $text .= '<tr>';
            $text .= '<td colspan="2"><img src="https://' .$_SERVER['HTTP_HOST']. '/images/email/logo_feedback.jpg" alt="Showcode. Обратная связь." title="Showcode. Обратная связь." style="margin: 0pt; padding: 0pt; border: 0pt none; display: block;"></td>';
	    $text .= '</tr>';
	    $text .= '<tr>';
	    $text .= '<td colspan="2"><div style="height: 40px;"><p style="font-family:Arial, Helvetica, sans-serif; font-size:24px; font-weight:normal; font-style:normal; color:#333; padding-bottom: 10px;">Пользователь <a href="https://' .$_SERVER['HTTP_HOST']. '/user/view/' .$this->user_id. '">' .Yii::app()->user->getAuthorName($this->user_id). '</a> отправил '.$typeD[$this->type].'</p></div></td>';
	    $text .= '</tr>';
	    $text .= '<tr>';
