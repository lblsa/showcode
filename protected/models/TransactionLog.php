...
class TransactionLog extends CActiveRecord
{
	...
	protected function beforeSave()
	{
		if(parent::beforeSave())
		{
			...
			if (count($this->errors)==0)
			{
                            ...
                            $matrixPointSize = 6;
                            if($event->online){
                                    $data='https://' .$_SERVER['HTTP_HOST']. '/ticket/view/' .$this->uniq;
                            }else{
                                    $data='https://' .$_SERVER['HTTP_HOST']. '/ticket/view/' .$this->uniq. '#rsa=' .$this->rsa;
                            }
	...
	protected function afterSave()
	{
		...
		if($this->type == 'free'){
			...
			if($this->mail && $this->payment != 'credit_card'){
                            ...
                            $text = $text.'<tr>';
                            $text = $text.'<td colspan="2"><img src="https://' .$_SERVER['HTTP_HOST']. '/images/email/logo_booking_ticket.jpg" alt="Showcode. Бронирование билета." title="Showcode. Бронирование билета." style="margin: 0pt; padding: 0pt; border: 0pt none; display: block;"></td>';
			    $text = $text.'</tr>';
			    $text = $text.'<tr>';
                            if(!$eventUniq)
				...
				$text = $text.'</b><br/><br/>Вы должны оплатить билет, выбранным вами способом, <b>в течение 4 часов. Если за это время оплата не произойдёт, то ваша бронь аннулируется.</p></td>';
                                ...
                                $text = $text.'<img src="https://' .$_SERVER['HTTP_HOST']. '/images/email/zoo/zoo-logo.png" alt="' .$event->title. '" title="' .$event->title. '" style="margin: 0pt; padding: 0pt; border: 0pt none; display: block; border:1px #cccccc solid;">';
                                $text = $text.'</td>';
                                ...
                                $text = $text.'<img style="vertical-align:bottom;" src="https://' .$_SERVER['HTTP_HOST']. '/images/email/zoo/bird_top_part.png" alt="" />';
                                $text = $text.'</td>';
                                $text = $text.'</tr>';
                                $text = $text.'<tr>';
                                $text = $text.'<td><img style="vertical-align:top;" src="httpss://' .$_SERVER['HTTP_HOST']. '/images/email/zoo/bird_bottom_part_empty.png" alt=""/></td>';
                                $text = $text.'<td><img style="vertical-align:top;" src="https://' .$_SERVER['HTTP_HOST']. '/images/email/zoo/bird_bottom_part_empty.png" alt=""/></td>';
                                $text = $text.'<td>';
                                $text = $text.'<img style="vertical-align:top;" src="https://' .$_SERVER['HTTP_HOST']. '/images/email/zoo/bird_bottom_part.png" alt=""/>';
                                $text = $text.'</td>';
                                $text = $text.'</tr>';
                                $text = $text.'</table>';
                                $text = $text.'</td>';
	...
	public function buyIsDone(){
		...
		if (isset($phone))
		{
			...
			$message = 'Ваш билет находится здесь:' .PHP_EOL. 'https://' .$_SERVER['HTTP_HOST']. '/ticket/view/' .$this->uniq. '?preview';
		...
		$text = $text.'<table cellspasing="0" border="0" cellpadding="0" width="100%" style="background-color:#dadada; border-collapse: collapse; border-spacing:0;">';
                ...
                $text = $text.'<table cellspasing="0" border="0" cellpadding="0" width="698px" style="margin: 0pt; padding: 0pt; background-color: rgb(255, 255, 255); border-collapse: collapse; border-spacing:0;">';
                $text = $text.'<tr>';
                $text = $text.'<td colspan="2"><img src="https://' .$_SERVER['HTTP_HOST']. '/images/email/logo_thank_you.jpg" alt="Showcode. Спасибо за покупку." title="Showcode. Спасибо за покупку." style="margin: 0pt; padding: 0pt; border: 0pt none; display: block;"></td>';
                $text = $text.'</tr>';
                $text = $text.'<tr>';
                if(!$eventUniq)
			...
			$text = $text.'). ';
                ...
                $text = $text.'<tr>';
                if(!$eventUniq)
                    $text = $text.'<td rowspan="3" width="120px"><a href="https://' .$_SERVER['HTTP_HOST']. '/ticket/view/' .$this->uniq. '" target="_blank" title=""><img src="https://' .$_SERVER['HTTP_HOST']. '' .$this->qr. '" alt="' .$event['title']. '" style="margin: 0pt; padding: 0pt; border: 0pt none; display: block; border:1px #cccccc solid;"></a></td>';
                else
                    $text = $text.'<td rowspan="3" width="120px"><a href="https://' .$_SERVER['HTTP_HOST']. '/ticket/view/' .$this->uniq. '" target="_blank" title=""><img src="https://' .$_SERVER['HTTP_HOST']. '' .$this->qr. '" alt="' .$event['title']. '" width="125" style="margin: 0pt; padding: 0pt; border: 0pt none; display: block; border:1px #cccccc solid;"></a></td>';
                $text = $text.'<td style="font-family:Arial, Helvetica, sans-serif; font-size:14px; font-weight:normal; font-style:normal; color:#000;">Номер билета:</td>';
                $text = $text.'<td style="font-family:Arial, Helvetica, sans-serif; font-size:14px; font-weight:normal; font-style:normal;">' .$this->uniq. '</td>';
                $text = $text.'</tr>';
                $text = $text.'<tr>';
                $text = $text.'<td style="font-family:Arial, Helvetica, sans-serif; font-size:14px; font-weight:normal; font-style:normal; color:#000;">Ссылка на билет:</td>';
                $text = $text.'<td style="font-family:Arial, Helvetica, sans-serif; font-size:14px; font-weight:normal; font-style:normal; text-decoration:underline; color:#0000ff;"><a target="_blank" href="https://' .$_SERVER['HTTP_HOST']. '/ticket/view/' .$this->uniq. '" title="Здесь вы можете просмотреть статус покупки">' .$_SERVER['HTTP_HOST']. '/ticket/view/' .$this->uniq. '</a></td>';
                $text = $text.'</tr>';
                $text = $text.'<tr>';
                $text = $text.'<td colspan="2" style="font-family:Arial, Helvetica, sans-serif; font-size:12px; font-weight:normal; font-style:normal; color:#999;">';
	...
	public function getTextEmailSendListTickets($tickets){
            ...
            $text = $text.'<td><img src="https://' .$_SERVER['HTTP_HOST']. '/images/email/logo_empty.jpg" alt="Showcode." title="Showcode." style="margin: 0pt; padding: 0pt; border: 0pt none; display: block;"></td>';
            $text = $text.'</tr>';
            ...
            $text = $text.'</tr>';
            $text = $text.'<tr>';
            $text = $text.'<td style="background-color:#e5e5e5;"><p style="padding:16px 10px; font-family:Arial, Helvetica, sans-serif; font-size:14px; font-weight:normal; font-style:normal; color:#333;">Вы запросили список всех билетов для мероприятия под названием «<a target="_blank" title="' .$this->title. '" href="https://' .$_SERVER['HTTP_HOST']. '/events/view/' .$this->id. '">' .$this->title. '</a>»</p></td>';
            $text = $text.'</tr>';
            $text = $text.'<tr>';
