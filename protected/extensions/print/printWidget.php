 ...
 class printWidget extends CWidget
{
    ...
    */
    ...
    public function init()
    {
        ...
        echo CHtml::link("<img src=\"$imgUrl\" class=\"img_link\" title=\"".$this->tooltip."\">" . $text, 
                        "", array_merge($this->htmlOptions, array('id' => $this->id,'onclick'=>'return processPrint();')));
        self::$count++;
        if (self::$count == 1) {
                ...
                function processPrint()
                {
                    if (firstTime) {
                        ...
                        for (var i=0; i<arrayEl.length; i++) {
                        	if(arrayEl[i].getElementsByTagName('a')[0])
                        		arrayEl[i].getElementsByTagName('a')[0].removeAttribute('href');
